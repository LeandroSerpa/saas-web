...
describe('BeachTennisTurmaAlunosView retorno do novo aluno', () => {
  it('exibe o cadastro no cabecalho somente apos carregar uma turma selecionada', () => {
    ...
    assert.match(source, /const turmaIdSelecionada = computed\(\(\) => normalizarIdInteiroPositivo\(route\.query\.turmaId\)\)/)
  })

  it('executa a navegacao para cadastro apenas com a query da turma selecionada', () => {
    ...
    assert.equal(normalizarIdInteiroPositivo(['2']), null)
  })

  it('mantem o CTA utilizavel no cabecalho em telas estreitas', () => {
  ...
  it('consome o aluno criado sem salvar automaticamente o vinculo', () => {
    ...
    assert.match(trecho, /estado\?\.novoAlunoId \?\? route\.query\.novoAlunoId/)
    ...
    assert.doesNotMatch(trecho, /valorRota\(/)
  })

  it('mantem os textos da tela de vinculacao de alunos sem mojibake', () => {
    assert.match(source, /Dispon/)
    assert.match(source, /Selecionar/)
    assert.match(source, /Limpar/)
    assert.match(source, /N.{0,2}vel/)
    assert.match(source, /Carregando .* dispon/)
    assert.match(source, /Nenhum .* dispon/)
    assert.match(source, /Desfazer/)
  ...
  it('usa apenas query para identificar a turma ativa', () => {
    ...
    assert.doesNotMatch(source, /valorRota\(/)
  })
})
